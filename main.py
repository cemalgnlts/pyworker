...
class Backend:
    def __init__(self) -> None:
        ...
        self._sig_format_name = None  # imza formatı bulununca sabitlenir
    ...
    async def wait_for_model_ready(self) -> None:
        ...
        self.metrics.model_loaded(max_throughput=WORKER_PERF)  # istek tabanlı: sabit perf
    ...
    def _verify_raw(self, message: str, sig_b64: str) -> bool:
        """Mesajı, imzayı hem standart hem urlsafe base64 ile dener."""
        if not message or not sig_b64:
            return False
        for decoder in (base64.b64decode, base64.urlsafe_b64decode):
            try:
                sig = decoder(sig_b64 + "=" * (-len(sig_b64) % 4))
    ...
    def check_signature(self, auth_data: AuthData) -> bool:
        ...
        if self._sig_format_name:
            for name, msg in self._sig_messages(auth_data):
                if name == self._sig_format_name and self._verify_raw(msg, sig_b64):
                    return True
        # Tüm adayları dene; tutanı sabitle
        for name, msg in self._sig_messages(auth_data):
            if self._verify_raw(msg, sig_b64):
                if self._sig_format_name != name:
                    log.info(f"✅ İmza doğrulandı ve format sabitlendi: '{name}'")
    ...
    def _debug_signature_formats(self, auth_data: dict) -> None:
        ...
        # *** DOĞRU PUBKEY TESTİ: api_key'in KENDİ imzasını KENDİ msg'i üzerinde doğrula ***
        # Bu, "pubkey doğru mu yanlış mı" sorusunu kesin yanıtlar.
        if api_key_raw:
            try:
                api_obj = json.loads(base64.b64decode(api_key_raw + "=" * (-len(api_key_raw) % 4)))
                api_msg, api_sig = api_obj.get("msg"), api_obj.get("signature")
                ok = False
                if api_msg and api_sig:
                    for lbl, m in [
                        ("ham", api_msg),
                        ("i4 sort", json.dumps(json.loads(api_msg), indent=4, sort_keys=True)),
                        ("compact sort", json.dumps(json.loads(api_msg), sort_keys=True)),
                    ]:
                        if self._verify_raw(m, api_sig):
                            log.info(f"  ✅ PUBKEY TESTİ GEÇTİ (api_key kendi imzası '{lbl}' ile) -> PUBKEY DOĞRU; sorun auth_data formatı/base64'ünde")
                            ok = True
                            break
                if not ok:
                    log.info("  ❌ PUBKEY TESTİ: api_key kendi imzası da doğrulanamadı -> PUBKEY YANLIŞ (yanlış /pubkey kaynağı)")
            except Exception as e:
                log.info(f"  PUBKEY TESTİ yapılamadı: {e}")

        found = False
        sig_b64 = auth_data["signature"]
        for label, message in candidates.items():
            if self._verify_raw(message, sig_b64):
