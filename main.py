...
class Backend:
    ...
    def check_signature(self, auth: AuthData) -> bool:
        if UNSECURED:
            return True
        if self._pubkey is None:
            log.error("İmza reddedildi: pubkey yok")
            return False
        message = json.dumps({"url": auth.url}, indent=4, sort_keys=True)
        try:
            pkcs1_15.new(self._pubkey).verify(SHA256.new(message.encode()), base64.b64decode(auth.signature))
            return True
